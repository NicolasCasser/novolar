...
export class InitialSchema1786716814145 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TYPE "public"."adoption_requests_state_enum" AS ENUM('AC', 'AL', 'AP', 'AM', 'BA', 'CE', 'DF', 'ES', 'GO', 'MA', 'MT', 'MS', 'MG', 'PA', 'PB', 'PR', 'PE', 'PI', 'RJ', 'RN', 'RS', 'RO', 'RR', 'SC', 'SP', 'SE', 'TO')`,
    );
    await queryRunner.query(
      `CREATE TYPE "public"."adoption_requests_status_enum" AS ENUM('PENDING', 'IN_ANALYSIS', 'APPROVED', 'REJECTED', 'CANCELED')`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `CREATE TYPE "public"."animals_species_enum" AS ENUM('DOG', 'CAT', 'BIRD', 'RABBIT', 'OTHER')`,
    );
    await queryRunner.query(
      `CREATE TYPE "public"."animals_sex_enum" AS ENUM('MALE', 'FEMALE')`,
    );
    await queryRunner.query(
      `CREATE TYPE "public"."animals_size_enum" AS ENUM('SMALL', 'MEDIUM', 'LARGE')`,
    );
    await queryRunner.query(
      `CREATE TYPE "public"."animals_state_enum" AS ENUM('AC', 'AL', 'AP', 'AM', 'BA', 'CE', 'DF', 'ES', 'GO', 'MA', 'MT', 'MS', 'MG', 'PA', 'PB', 'PR', 'PE', 'PI', 'RJ', 'RN', 'RS', 'RO', 'RR', 'SC', 'SP', 'SE', 'TO')`,
    );
    await queryRunner.query(
      `CREATE TYPE "public"."animals_status_enum" AS ENUM('AVAILABLE', 'ADOPTED')`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `CREATE TABLE "animal_images" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "created_at" TIMESTAMP NOT NULL DEFAULT now(), "updated_at" TIMESTAMP NOT NULL DEFAULT now(), "animal_id" uuid NOT NULL, "url" character varying NOT NULL, "is_primary" boolean NOT NULL, CONSTRAINT "PK_401733200e1009acda47938bbc9" PRIMARY KEY ("id"))`,
    );
    await queryRunner.query(
      `ALTER TABLE "adoption_requests" ADD CONSTRAINT "FK_9738e0e44f3c2f6445e986fccfe" FOREIGN KEY ("animal_id") REFERENCES "animals"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`,
    );
    await queryRunner.query(
      `ALTER TABLE "animals" ADD CONSTRAINT "FK_66f0b544932f22f1d04214e61d4" FOREIGN KEY ("created_by_user_id") REFERENCES "users"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`,
    );
    await queryRunner.query(
      `ALTER TABLE "animal_images" ADD CONSTRAINT "FK_dfb5d6007baa2b465465160ed5b" FOREIGN KEY ("animal_id") REFERENCES "animals"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`,
    );
  }

  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE "animal_images" DROP CONSTRAINT "FK_dfb5d6007baa2b465465160ed5b"`,
    );
    await queryRunner.query(
      `ALTER TABLE "animals" DROP CONSTRAINT "FK_66f0b544932f22f1d04214e61d4"`,
    );
    await queryRunner.query(
      `ALTER TABLE "adoption_requests" DROP CONSTRAINT "FK_9738e0e44f3c2f6445e986fccfe"`,
    );
    ...
    await queryRunner.query(
      `DROP TYPE "public"."adoption_requests_state_enum"`,
    );
